...
plot_state_ts <- function(s) {
  ...
  if(abs(current_rates[1] - current_rates[2]) < 0.03) {
    current_rates[current_rates == min(current_rates)] <-  min(current_rates) - 0.012
    current_rates[current_rates == max(current_rates)] <- max(current_rates) + 0.012
  }
  p <- plot_data %>%
    ggplot() +
    aes(x = date, y = mean, ymin = lower, ymax = upper,
  ...
  annotate("text", x = max(state_ts$date), current_rates,
...
for (s in p_biden$state) {
  ...
  ptrump <- case_when(ptrump < 0.01 ~ "less than 1%",
   ...
   pander::pandoc.p(paste0('Biden is projected to receive **between ',
                           ...
                           ptrump, '**.  Dotted lines represent each party\'s vote share in the previous presidential election.'))
   plot_state_ts(s)
   pander::pandoc.p('\n')
}
```
