...
for (s in p_biden$state) {
  ...
  ptrump <- case_when(ptrump < 0.01 ~ "less than 1%",
   ...
   pander::pandoc.p('\n')
   pander::pandoc.p(paste0('Biden is projected to receive ',
                           ...
                           '% of the vote in ', s, '.  (average = ',
                           round(100*state_ts$mean_biden[state_ts$state == s 
                               & state_ts$date == max(state_ts$date)], 0), '%) ',
                           ...
                           & state_ts$date == max(state_ts$date)], 0), 
                           '% (average = ', round(100*state_ts$mean_biden[state_ts$state == s 
                               & state_ts$date == max(state_ts$date)], 0), '%) ',
